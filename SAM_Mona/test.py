...
def parse_args():
    ...
    parser.add_argument("--test_dataset_path", type=str, default="/root/autodl-tmp/datasets/test_dataset/10028/denoised/*.jpg", help="test data path")
    ...
    parser.add_argument("--sam_checkpoint", type=str, default="/root/autodl-tmp/checkpoint/SAM_Med2D_mona_mask1/models/sam_best_10093.pth", help="sam checkpoint")
    # /root/Promptpoint/SAM-Med2D/workdir/test1/models/sam_best.pth
    # parser.add_argument("--sam_checkpoint", type=str, default="/root/Promptpoint/SAM-Med2D/workdir/test1/models/sam_best.pth", help="sam checkpoint")
...
def main(args):
    ...
    # test_image_path = test_image_path[30:]
    args.prompt_path = args.test_point_path
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        # batched_input["point_coords"]= batched_input["point_coords"].squeeze(0)
        # batched_input["point_labels"]= batched_input["point_labels"].squeeze(0)
        
        # print(batched_input["image"].shape, batched_input["label"].shape)
        # torch.Size([1, 3, 1024, 1024]) torch.Size([1, 1, 1024, 1024])
        # print(batched_input["point_coords"].shape)
        # torch.Size([1, 392, 2])        
        ...
        if args.boxes_prompt:
            ...
            save_path = os.path.join(f"{args.work_dir}", "results")
            ...
            for iter in range(args.iter_point):
                ...
                #     point_num = batched_input["point_coords"].shape[:1]
                #     point_num = point_num[0]
                    
                #     image_embed = image_embed.repeat(point_num, 1, 1, 1)
            ...
            # points_show = (torch.concat(batched_input["point_coords"], dim=1), torch.concat(batched_input["point_labels"], dim=1))
            points_show = (batched_input["point_coords"], batched_input["point_labels"])

        masks, pad = postprocess_masks(low_res_masks, args.image_size, original_size)
        # print("masks", masks.shape)
        # masks torch.Size([1, 1, 4096, 4096])
