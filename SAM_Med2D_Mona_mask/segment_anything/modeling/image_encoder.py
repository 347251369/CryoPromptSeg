...
class Adapter_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0,3,1,2)
        B, C, _, _ = x.size()
        # 对 x 进行全局平均池化（Global Average Pooling），得到 (B, C, 1, 1)
...
class MonaAdapter(nn.Module):
    def __init__(self,
        ...
        self.adapter_conv = MonaOp(hidden_dim)
    ...
    def forward(self, x, hw_shapes=None):
        ...
        # TODO  
        # print('mona',x.shape)
        # mona torch.Size([2, 64, 64, 768])

        x = self.norm(x) * self.gamma + x * self.gammax

        x = self.project1(x)# (B, H, W, inner_dim)

        x = x.permute(0, 3, 1, 2) 
        x = self.adapter_conv(x)
        x = x.permute(0, 2, 3, 1)# (B, H, W, inner_dim)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if self.adapter:
            ...
            self.mona1 = MonaAdapter(dim, 4)
            self.mona2 = MonaAdapter(dim, 4)
