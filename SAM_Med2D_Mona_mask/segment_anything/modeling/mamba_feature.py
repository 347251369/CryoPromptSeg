...
class PatchEmbed2D(nn.Module):
    ...
    def forward(self, x):
        # print("patch",x.shape)
        # patch torch.Size([2, 1, 1024, 1024])
        x = self.proj(x)
        x = x.permute(0, 2, 3, 1)
        # print('patchEmbed',x.shape)
        # patchEmbed torch.Size([2, 64, 64, 256])
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1,depths=[2],
                 dims=[512], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
        ...
        # 用于生成模型中各层的随机深度（stochastic depth）
