...
# 一个用于创建和配置日志记录器的函数 get_logger
# 支持同时将日志输出到文件和控制台，并可以根据需要调整日志级别。
def get_logger(filename, verbosity=1, name=None):
  # 将整数（0, 1, 2）映射到日志级别（DEBUG, INFO, WARNING）。
...
def save_masks(preds, save_path, img_name, image_size, original_size, pad=None,  boxes=None, points=None, visual_prompt=True):
    ...
    file_name = img_name.split('/')[5]
    # print("file",file_name)
    folder_path = f'{save_path}/{file_name}/masks'
    os.makedirs(folder_path, exist_ok=True)
    mask_name=img_name.split('/')[-1].replace('.jpg', '_mask.jpg')
    mask_path = f'{folder_path}/{mask_name}'
...
def collate_fn(batch):
    """
    batch: List[Dict], 每个 Dict 是一个 image_input
    返回 batched 字典
    """
    images = torch.stack([sample["image"] for sample in batch])
    labels = torch.stack([sample["label"] for sample in batch])
    pro_masks = torch.stack([sample["pro_mask"] for sample in batch])

    point_coords = [sample["point_coords"] for sample in batch]   # 不拼接，保持 list[Tensors]
    point_labels = [sample["point_labels"] for sample in batch]   # 同上
